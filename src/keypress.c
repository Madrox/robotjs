...
void toggleKeyCode(MMKeyCode code, const bool down, MMKeyFlags flags)
{
	...
	if (down)
	{
		...
		if (flags & MOD_ALT && flags & MOD_RIGHT)
			WIN32_KEY_EVENT_WAIT(K_RIGHT_ALT, dwFlags);
		else if (flags & MOD_ALT)
			WIN32_KEY_EVENT_WAIT(K_ALT, dwFlags);

		if (flags & MOD_CONTROL && flags & MOD_RIGHT)
			WIN32_KEY_EVENT_WAIT(K_RIGHT_CONTROL, dwFlags);
		else if (flags & MOD_CONTROL)
			WIN32_KEY_EVENT_WAIT(K_CONTROL, dwFlags);

		if (flags & MOD_SHIFT && flags & MOD_RIGHT)
			WIN32_KEY_EVENT_WAIT(K_RIGHTSHIFT, dwFlags);
		else if (flags & MOD_SHIFT)
	...
	else
	{
		...
		if (flags & MOD_ALT && flags & MOD_RIGHT)
			win32KeyEvent(K_RIGHT_ALT, dwFlags);
		else if (flags & MOD_ALT)
			win32KeyEvent(K_ALT, dwFlags);

		if (flags & MOD_CONTROL && flags & MOD_RIGHT)
			win32KeyEvent(K_RIGHT_CONTROL, dwFlags);
		else if (flags & MOD_CONTROL)
			win32KeyEvent(K_CONTROL, dwFlags);

		if (flags & MOD_SHIFT && flags & MOD_RIGHT)
			win32KeyEvent(K_RIGHTSHIFT, dwFlags);
		else if (flags & MOD_SHIFT)
	...
	if (down)
	{
		...
		if (flags & MOD_ALT && flags & MOD_RIGHT)
			X_KEY_EVENT_WAIT(display, K_RIGHT_ALT, is_press);
		else if (flags & MOD_ALT)
			X_KEY_EVENT_WAIT(display, K_ALT, is_press);

		if (flags & MOD_CONTROL && flags & MOD_RIGHT)
			X_KEY_EVENT_WAIT(display, K_RIGHT_CONTROL, is_press);
		else if (flags & MOD_CONTROL)
			X_KEY_EVENT_WAIT(display, K_CONTROL, is_press);

		if (flags & MOD_SHIFT && flags & MOD_RIGHT)
			X_KEY_EVENT_WAIT(display, K_RIGHTSHIFT, is_press);
		else if (flags & MOD_SHIFT)
	...
	else
	{
		...
		if (flags & MOD_ALT && flags & MOD_RIGHT)
			X_KEY_EVENT(display, K_RIGHT_ALT, is_press);
		else if (flags & MOD_ALT)
			X_KEY_EVENT(display, K_ALT, is_press);

		if (flags & MOD_CONTROL && flags & MOD_RIGHT)
			X_KEY_EVENT(display, K_RIGHT_CONTROL, is_press);
		else if (flags & MOD_CONTROL)
			X_KEY_EVENT(display, K_CONTROL, is_press);

		if (flags & MOD_SHIFT && flags & MOD_RIGHT)
			X_KEY_EVENT(display, K_RIGHTSHIFT, is_press);
		else if (flags & MOD_SHIFT)
